...
/*
  ...
  * your spindle is Makita RT0700 or Dewalt 611
  ...
  25/JUL/2018 - V7 : Code review to eliminate a bug causing GRBL Error 33,
                     Compared this postProcessor with the latest grbl.cps
  16/NOV/2018 - V8 : Added extra coolant modes : Flood = M8, Mist = M7, Flood+Mist
*/
/* eslint-disable no-unused-vars */
const description = 'Open Hardware CNC Router';
const longDescription = '';
const vendor = '';
const vendorUrl = '';
const model = 'Lead CNC';
const legal = 'Copyright (C) 2012-2018 by Autodesk, Inc.';
...
// end-of-line type : default for Windows OS is CRLF (so that's why this line is commented out), change to CR, LF, CRLF or LFCR if you are on another OS...
setEOL(CRLF);
...
const gFormat = createFormat({ prefix: 'G', decimals: 0 });
const mFormat = createFormat({ prefix: 'M', decimals: 0 });

const xyzFormat = createFormat({ decimals: (unit == MM ? 4 : 6) });
const feedFormat = createFormat({ decimals: (unit == MM ? 1 : 3) });
const rpmFormat = createFormat({ decimals: 0 });
const secFormat = createFormat({ decimals: 1, forceDecimal: true, trim: false });
const taperFormat = createFormat({ decimals: 1, scale: DEG });

const xOutput = createVariable({ prefix: 'X' }, xyzFormat);
const yOutput = createVariable({ prefix: 'Y' }, xyzFormat);
const zOutput = createVariable({ prefix: 'Z' }, xyzFormat);
const feedOutput = createVariable({ prefix: 'F' }, feedFormat);
const sOutput = createVariable({ prefix: 'S', force: true }, rpmFormat);

const iOutput = createReferenceVariable({ prefix: 'I' }, xyzFormat);
const jOutput = createReferenceVariable({ prefix: 'J' }, xyzFormat);
const kOutput = createReferenceVariable({ prefix: 'K' }, xyzFormat);

const gMotionModal = createModal({}, gFormat); 											// modal group 1 // G0-G3, ...
const gPlaneModal = createModal({ onchange() { gMotionModal.reset(); } }, gFormat); // modal group 2 // G17-19
const gAbsIncModal = createModal({}, gFormat); 											// modal group 3 // G90-91
const gFeedModeModal = createModal({}, gFormat); 											// modal group 5 // G93-94
const gUnitModal = createModal({}, gFormat); 												// modal group 6 // G20-21
...
function rpm2dial(rpm) {
  ...
  const speeds = [0, 10000, 12000, 17000, 22000, 27000, 30000];
...
function onOpen() {
  ...
  const myMachine = getMachineConfiguration();
  ...
  for (let i = 0; i < numberOfSections; ++i) {
    ...
    const rpm = section.getMaximumSpindleSpeed();
    ...
    let machineTimeInSeconds = section.getCycleTime();
    const machineTimeHours = Math.floor(machineTimeInSeconds / 3600);
    machineTimeInSeconds %= 3600;
    const machineTimeMinutes = Math.floor(machineTimeInSeconds / 60);
    const machineTimeSeconds = Math.floor(machineTimeInSeconds % 60);
    let machineTimeText = '  Machining time : ';
...
function onSection() {
  const nmbrOfSections = getNumberOfSections();		// how many operations are there in total
  var sectionId = getCurrentSectionId();			// what is the number of this operation (starts from 0)
  const section = getSection(sectionId);			// what is the section-object for this operation


  // check RadiusCompensation setting
  const radComp = getRadiusCompensation();
  ...
  let comment = `Operation ${sectionId + 1} of ${nmbrOfSections}`;
  ...
  const remaining = currentSection.workPlane;
  ...
  const initialPosition = getFramePosition(currentSection.getInitialPosition());
...
function onRapid(_x, _y, _z) {
  const x = xOutput.format(_x);
  const y = yOutput.format(_y);
  const z = zOutput.format(_z);
  if (x || y || z) {
...
function onLinear(_x, _y, _z, feed) {
  ...
  const y = yOutput.format(_y);
  const z = zOutput.format(_z);
  const f = feedOutput.format(feed);
